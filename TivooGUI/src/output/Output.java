...
public abstract class Output {
	...
	public String eventFile(String string, int i, int j, CalendarEvent cal) {
		...
		H1 head = new H1().appendChild(
				new Text("Detail Information about the Event:")).setAlign(
				"center");
		body.appendChild(head);
		Div div = new Div();
		cal.appendInformation(div); // Append eventName, date, location and
									// times to div.
		A link = new A().setHref("http://" + cal.getMyLink())
				.setTarget("_blank")
				.appendChild(new Text("Link for Detail Page")); // attach link
																// page
		...
		String preAddress = string + i + j + ".htm"; // construct filename for
														// specific event
