 ...
 *Events time within a month. output into html file
 */
public class MonthOutput extends Output {
		...
		for (int i = 1; i <= 31; i++) {
			...
			if (!myEventMap.get(i).isEmpty()) {
				for (int j = 0; j < myEventMap.get(i).size(); j++) {
					...
					Ul ul = new Ul();
					Li li = new Li();
					A link = new A().setHref(eventFile("Month", i, j, c))
							.setTarget("_blank");
	...
	/*
	 * Sort CalendarEvent into Map according to each day in a month. Used in
	 * Constructor
	 */
	private void constructMap() {
		for (CalendarEvent cal : myCalendar) { // Sort date into each day
			for (int j = 1; j <= 31; j++) {
				if ((cal.getMyStartTime().getDayOfMonth() == j)
