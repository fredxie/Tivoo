...
/*
 *SortListOutput class receive list of CalendarEvent objects with information of eventName, date, location, 
 *startTime and endTime.
 *Events been sorted in certain pattern, output into html file.
 */
public class SortListOutput extends Output {
	...
	/*
	 * Takes in a Body object, initialize the title by kind of sort Used in
	 * outputFile method.
	 */
	private Body constructDayFrame(Body body) {
		H1 title = new H1().setAlign("center");
		body.appendChild(title);
		title.appendChild(new Text("Sort Result"));
		return body;
	}

	/*
	 * Takes in a Body object, append Events within a day, return the body. Used
	 * in outputFile method.
	 */
	private Body addEventList(Body body) {
		...
		for (int i = 0; i < myCalendar.size(); i++) {
			...
			A link = new A().setHref(eventFile("Sort", i, 1, c)).setTarget(
					"_blank");
