...
public class Dialogue extends JFrame implements ActionListener {
	...
	private void makeLabel() {
		if (myType.equals("keyword")) {
			JLabel label = new JLabel("Please input keyword (In first line): ");
			label.setBounds(20, 10, 400, 20);
			panel.add(label);
		} else if (myType.equals("removeKeyword")) {
			JLabel label = new JLabel(
					"Please input removing keyword (In first line): ");
			label.setBounds(20, 10, 400, 20);
			panel.add(label);
			...
			JLabel label1 = new JLabel(
					"Please type in actor/actress name (TV available only): ");
			label1.setBounds(20, 10, 400, 20);
			panel.add(label1);
		}
	}
	...
	private void filterTime() throws Exception {
		...
		startTime[1] = Integer.parseInt(st[1]);
		startTime[2] = Integer.parseInt(st[2]);
		endTime[0] = Integer.parseInt(et[0]);
		endTime[1] = Integer.parseInt(et[1]);
		...
		myModel.setCalendar(processor.filter("timeFrame", l,
				myModel.getCalendar()));
	}
	...
	private void filterClass() throws Exception {
		...
		ArrayList<Object> l = new ArrayList<Object>();
		l.add(text1.getText());
		myModel.setCalendar(processor.filter("classSpecific", l,
				myModel.getCalendar()));
